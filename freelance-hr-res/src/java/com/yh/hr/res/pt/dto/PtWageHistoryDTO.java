...
public class PtWageHistoryDTO {
	...
	//工资项集合
	private List<PtWageHistoryItemsDTO> ptWHistoryItemslist;
	//工资项Map
	private Map<String,PtWageHistoryItemsDTO> ptWHistoryItemsMap;
	
	public List<PtWageHistoryItemsDTO> getPtWHistoryItemslist() {
		return ptWHistoryItemslist;
	}
	public void setPtWHistoryItemslist(
			List<PtWageHistoryItemsDTO> ptWHistoryItemslist) {
		this.ptWHistoryItemslist = ptWHistoryItemslist;
		buildWageItemMap(ptWHistoryItemslist);
	}
	 ...
	 * @param wageItemList
	 */
	private void buildWageItemMap(List<PtWageHistoryItemsDTO> ptWHistoryItemslist)
	{
		//先清空wageItemMap，或者初始化
		if(null != ptWHistoryItemsMap) 
		{
			ptWHistoryItemsMap.clear();	
		}
		else
		{
			ptWHistoryItemsMap = new HashMap<String, PtWageHistoryItemsDTO>();
		}
		//设置工资项Map
		if(CollectionUtils.isNotEmpty(ptWHistoryItemslist))
		{
			for (PtWageHistoryItemsDTO PtWHistoryItemsDTO : ptWHistoryItemslist)
			{
				ptWHistoryItemsMap.put(PtWHistoryItemsDTO.getWageItemCode(), PtWHistoryItemsDTO);
			}
		}
	}
	//根据工资项编码从工资项Map中获取到工资项DTO     PtWageHistoryItems
	public PtWageHistoryItemsDTO getPtWHistoryItems(String wageItemCode)
	{
		if(null != ptWHistoryItemsMap)
		{
			return ptWHistoryItemsMap.get(wageItemCode);
		}
		return null;
	}
	/**
	 ...
	 * @param wageItem
	 */
	public void addWageItem(PtWageHistoryItemsDTO ptWHistoryItemsDTO)
	{
		if(CollectionUtils.isEmpty(ptWHistoryItemslist))
		{
			ptWHistoryItemslist = new ArrayList<PtWageHistoryItemsDTO>();
		}
		ptWHistoryItemslist.add(ptWHistoryItemsDTO);
		addToWageItemMap(ptWHistoryItemsDTO);
	}
	/**
	 * 往工资项MAP中新增工资项
	 * @param wageItem
	 */
	private void addToWageItemMap(PtWageHistoryItemsDTO ptWHistoryItemsDTO)
	{
		if(null == ptWHistoryItemsMap)
		{
			ptWHistoryItemsMap = new HashMap<String, PtWageHistoryItemsDTO>();
	...
	public Map<String, PtWageHistoryItemsDTO> getPtWHistoryItemsMap() {
		return ptWHistoryItemsMap;
	}
	public void setPtWHistoryItemsMap(
			Map<String, PtWageHistoryItemsDTO> ptWHistoryItemsMap) {
		this.ptWHistoryItemsMap = ptWHistoryItemsMap;
	}
