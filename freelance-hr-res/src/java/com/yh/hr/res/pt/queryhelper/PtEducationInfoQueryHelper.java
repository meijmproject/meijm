 ...
 */
public class PtEducationInfoQueryHelper
{
	/**
	 * 通过主键获取
	...
	/**
	 * 通过bizPersonOid以及educationCode获取
	 * @param bizPersonOid
	 * @param educationCodeList
	 * @return
	 * @throws ServiceException
	 */
	public static List<PtEducationInfoDTO> listPtEducationDTOByEducationCode(Long bizPersonOid, List<String> educationCodeList) throws ServiceException
	{
		String hql = "from PtEducationInfo ei where ei.bizPersonOid = :bizPersonOid and ei.educationCode in (:educationCodeList) order by ei.graduateDate desc";
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("bizPersonOid", bizPersonOid);
		params.put("educationCodeList", educationCodeList);
		List<PtEducationInfo> boList = DaoUtil.find(hql, params);
		List<PtEducationInfoDTO> dtoList = new ArrayList<PtEducationInfoDTO>();
		if(CollectionUtils.isNotEmpty(boList))
		{
	 ...
	 */
	public static void updateIsHighFlag(Long bizPersonOid,Long ptEducationOid,boolean flag) throws ServiceException{
		...
		hql.append(" where  ei.ptEducationOid <> " +ptEducationOid);
	...
	/**
	 * 当flag为true时，获取最高学历，false获取最高学位
	 */
	public static PtEducationInfo getHighEducationInfo(Long bizPersonOid,boolean flag) throws ServiceException{
		final StringBuffer hql = new StringBuffer(" from  PtEducationInfo ei where 1 = 1");
		if(flag){
			hql.append(" and ei.isHighestEducationLevel = '" +Constant.YES+ "'");
		}else{
			hql.append(" and ei.isHighestDegree = '" +Constant.YES+ "'");
		}
		hql.append(" and   ei.bizPersonOid =  " +bizPersonOid);
		List<PtEducationInfo>  ptEducationInfos = DaoUtil.find(hql.toString());
		return CollectionUtils.isNotEmpty(ptEducationInfos) ? ptEducationInfos.get(0) : null;
	}
}
