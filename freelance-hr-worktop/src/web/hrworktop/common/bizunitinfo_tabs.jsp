			...
			if(data.success==false){
				...
				for(var i=0;i<groups.length;i++){
					...
					for(var j=0;j<group.length;j++){
						...
						div+="<li id='li"+indexId+"' name='"+infoUrl+"'><a href='#"+indexId+"' name='"+indexId+"' onclick=findPageUrl('"+indexId+"','"+utUnitOid+"','"+infoUrl+"')>"+group[j].libraryName+"</a></li>";
				    }
				}
				div+="</ul>";
				...
				liAyy.each(function(){
				   ...
				   findPageUrl(liId,utUnitOid,liName,false);
				}) 
			}
		}
	});
...
function findPageUrl(id,utUnitOid,infoUrl,flag){
		$("#" + id).attr('url',infoUrl);
		$("#" + id).load(infoUrl,{"utUnitOid":utUnitOid,"Id":id},function(){
			if(flag!==false){
				var pos = $("#" + id).offset();
