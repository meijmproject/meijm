...
<!--
 * @function    录入审批不同意意见页面
 * @page name   /freelance-hr-worktop/src/web/hrworktop/disAgree_opinion.jsp
 * @author      wangx
 * @created     2017-05-17
 * @version     1.0
-->
	...
	$(".agree_opinion_submit").click(function(){
	    ...
	    $("#formAgreeOpinion").validate({
	        ...
	        submitHandler: function (form)
	        {
	    		...
	    		var date = $('#date').val();
				Mask.showMask();
	    		$.post("bizDefaultRecheckDisAgree.do?method=submitRecheckDisAgree&itemCodes="+itemCodes+"&itemNodeCodes="+itemNodeCodes+"&approveLeader="+leader+"&approveDate="+date+"&opinion="+opinion+"&bizTaskOids="+bizTaskOids+"&applyNames="+applyNames,function(data){
					Mask.hideMask();
	       			if (data.success) {
	       				Widget.close();
    ...
    <div>
       ...
       <%-- <p class="agree_opinion_fontTop">本次业务涉及以下人员：</p>
        ...
        <div class="agree_opinion_inputfont">
            <textarea id="opinion" name="content" class="z_text_id" maxlength="200" rows="13" cols="125%;" placeholder="请输入审核意见；(字数不能超过200字)" />
        </div>
        <p class="agree_opinion_alert">
            <span class="agree_opinion_alertred">*</span>
            <span class="agree_opinion_alertfont">你已经输入了 <span class="agree_opinion_red">0</span> 个字</span>
