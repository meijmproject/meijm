...
$.extend({
    newCreateOrgTree:function(data,divIdName){
        var parentNodeArr=findParentNodes(data);
        createOrganizationTree(data);
        $(document).on('click','.Unfold',function (event) {
            unfoldOrFold(event);
        });
        function  findParentNodes(data) {
            var tempArr = []
            var allNodeArr = [];
            var parentNodeArr = [];
            for(var i=0;i<data.length;i++){
                allNodeArr.push(data[i].nodeName);
                if(data[i].parentNode!== ''){
                    tempArr.push(data[i].parentNode);
                }
            }
            for(var j=0;j<tempArr.length;j++){
                if(!contains(parentNodeArr,tempArr[j])){
                    parentNodeArr.push(tempArr[j]);
                }
            }
            return parentNodeArr;
        }
        function unfoldOrFold(event){
            var thisDome = event.currentTarget;
            $thisDome = $(thisDome);
            var domname = $thisDome.parent().attr('name');
            for(var i=0;i<data.length;i++){
                if(data[i].nodeName == domname){
                    data[i].Unfold = !data[i].Unfold;
                    break;
                }
            }
            $('#'+divIdName).empty();
        ...
        function unfoldOrFoldIcon(arr){
            for(var i=0;i < arr.length;i++){
                if(arr[i].getAttribute('name') == 'true'){
                    arr[i].firstChild.setAttribute('class','delete');
                }else{
                    arr[i].firstChild.setAttribute('class','add');
                }
            }
        }
        function createOrganizationTree(jsonArray){
            var  node=[];
            for(var i=0;i < jsonArray.length;i++){
                node[i] = new OrgNode();
                node[i].nodeName = jsonArray[i].nodeName;
                node[i].parentNode = jsonArray[i].parentNode;
                node[i].customParam.EmpName = jsonArray[i].EmpName;
                node[i].customParam.department = jsonArray[i].department;
                node[i].customParam.EmpPhoto = jsonArray[i].EmpPhoto;
                node[i].customParam.Url = jsonArray[i].Url;
                node[i].customParam.domName = jsonArray[i].nodeName;
                node[i].customParam.Unfold = jsonArray[i].Unfold;
                if(contains(parentNodeArr,jsonArray[i].nodeName)){
                    node[i].customParam.addClass = 'Unfold';
                }else {
                    node[i].customParam.addClass = '';
                }
                if(node[i].parentNode !=''){
                    for(var j=0;j<i;j++){
                        if(node[j].nodeName == node[i].parentNode ){
                            if(node[j].customParam.Unfold == true){
                                node[j].Nodes.Add(node[i]);
                            }
                        }
                    }
                }
            }
            var OrgShows=new OrgShow(node[0]);
            OrgShows.Top=50;
            OrgShows.Left=50;
            OrgShows.IntervalWidth=10;
            OrgShows.IntervalHeight=20;
            OrgShows.BoxTemplet="<div id=\"{Id}\" class=\"OrgBox\" name=\"{domName}\"><div>{EmpName}</div><p>{department}</p><p>{EmpPhoto}</p><div class=\"{addClass}\" name=\"{Unfold}\"><i alt=''></i></div></div>"
            OrgShows.Run();
            unfoldOrFoldIcon($('.Unfold'));
        };
        ...
        function findDomName(name){return document.getElementsByName(name)};
        ...
        function OrgNode(){
            ...
            this.inIt= function(){
                ...
                findId(divIdName).appendChild(tempDiv);
        ...
        function OrgShow(OrgNode_){
            ...
            function CreateLine(type_, top_, left_, long_, size_, color_, id_){
                ...
                findId(divIdName).appendChild(this.Line);
