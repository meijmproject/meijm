 ...
 * 
 */
package com.yh.hr.info.biz.bizuploadphoto.queryhelper;
...
public class BizUploadPhotoQueryHelper {


	/**
	 * 查询文件列表，不分页
	 * @param ttb（bizPersonOid 必填）
	 * @return
	...
	public static List<JSONObject> listUploadPhoto(TableTagBean ttb) throws ServiceException {
		...
		if (StringUtils.isEmpty(bizPersonOid)) {
			return null;
		}
...
//		sql.append("       ,(select to_char(wm_concat(jura.authority)) from yha_upload_ref_auth jura where jura.ref_code = juf.ref_code and jura.ref_role_code = '").append(bizPersonOid).append("')");
		...
		sql.append("  from yhc_pt_photo jpp left join yha_upload_status jus");
		sql.append(" on jpp.photo_code = jus.file_name_remote");
		sql.append(" where jpp.biz_person_oid ="+bizPersonOid);
		if(!"0".equals(refType)){
...
//		sql.append(" and exists(select 1 from yha_upload_ref_auth jura where jura.ref_code = juf.ref_code and jura.ref_role_code = '").append(bizPersonOid).append("') ");
		...
		return BeanHelper.copyProperties(list, new BeanHelper.PropertiesHandler<Object[], JSONObject>() {
			...
			public JSONObject getValue(Object[] src) throws ServiceException {
				...
				json.put("bizPersonOid", DataConverUtils.toNumber(src[1]));
