...
<script src="hrinfo/bizUploadPhoto/js/UploadPhotoUtils.js" type="text/javascript"></script>
...
<div id="transaction_modal_box" class="transaction_modal_box">
    ...
    <div class="modal-dialog-container">
        ...
        <div class="md-main-content m-file-upload">
            ...
            <span class="upload-tips">请上传业务材料<span>（支持doc,docx,xls,xlsx,zip,pdf,jpg,png,tif文件类型，不得超过5MB）</span></span>
...
(function($, window, document){
	...
	$('#fileuploadDomId').fileupload({
		...
		add : function(e, data) {
			...
			data.context = $(buff.join('')).prependTo('#fileuploadListId table >tbody');
			...
			if (errorMessage.length > 0) {
				...
				UploadPhotoUtils.md5({
					...
					complete : function(md5, fileName, file, options) {
						...
						//构建url
						var url = 'bizUploadPhoto.do?method=uploadPhoto&bizPersonOid=${bizPersonOid}&refType=${refType}';
						url = url + '&uuid='+uuid;
						url = url + '&fileMD5='+md5;
		...
		progress : function(e, data) {
			...
			if (data.result.postComplete === 1) {
				data.context.attr('photoOid', data.result.photoOid);
				label.html('上传完成');
				...
				data.context.find('[button-click=delete]').click(function(){
					deleteFile.call(this, data.result.photoOid, data.context);
				});
				data.context.find('[button-click=download]').click(function(){
					downloadFile.call(this, data.result.photoOid, data.context);
				});
			}
	...
	$.get(
		'listBizUploadPhoto.do?method=listUploadPhoto&bizPersonOid=${param.bizPersonOid}&refType=${param.refType}',
		function(data){
			$.each(data.rows,function(index, file){
				var authorities = file.authority.split(',');
				
				var buff = [
					'<tr  photoOid="'+file.photoOid+'">',
						'<td style="width:40%!important;" name="name" title="'+file.photoName+'">',
						'<span class="file-icon file-',(file.photoType?file.photoType.toLowerCase():''),'"></span>'
						,file.photoName,'</td>',
						'<td style="width:15%!important;" title="'+$.formatNumber(file.fileLength/(1024*1024),2,'.','')+'MB">',
				...
				if ($.inArray('9', authorities) != -1) {
					...
					context.find('[button-click=delete]').click(function(){
						deleteFile.call(this, file.photoOid, context);
					});
				}
				
				context.find('[button-click=download]').click(function(){
					downloadFile.call(this, file.photoOid, context);
				});
	...
	function deleteFile(photoOid, context) {
		MessageBox.yes('提示','请确认是否删除?', function(){
			$.get('deleteBizUploadPhoto.do?method=deleteUploadPhoto&photoOid='+photoOid, function(data) {
	...
	function downloadFile(photoOid) {
		MessageBox.openWindow('downloadBizUploadPhoto.do?method=downloadUploadPhoto&photoOid='+photoOid);
	};
