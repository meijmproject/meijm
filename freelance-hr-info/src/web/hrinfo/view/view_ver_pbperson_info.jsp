			...
			<h2 class="infoshow-title">
				...
				<c:if test="${flag ne '1'}">
					...
					<c:if test="${updateFlag ne '2'}">
						<button href="goToUpdateVerPersonPage.do?method=goToUpdateVerPersonPage&personOid=${param.personOid}&id=${param.Id}" class="popdown btn-modify btn-left-icon btn-default btn-right">修改</button>
					</c:if>
		...
		<div class="infoshow-container multi-info-group clearfix">
			...
			<div class="data-units">
				...
				<%-- <dl class="data-unit-dl">
					<dt>现任职务所在科室：</dt>
					<dd>
						<label title='<bean:write name="verPbPersonInfoForm" property="dutyDeptName" />'> <bean:write name="verPbPersonInfoForm" property="dutyDeptName" />&nbsp;</label>
					</dd>
				</dl>
				<dl class="data-unit-dl">
					<dt>现任职务：</dt>
					<dd>
						<label title='<dictionary:viewDicItemName dicTypeCode="<%=DicConstants.YHRS0126%>" dicItemCode="${verPbPersonInfoForm.dutyName}" />'> 
						<dictionary:viewDicItemName dicTypeCode="<%=DicConstants.YHRS0126%>" dicItemCode="${verPbPersonInfoForm.dutyName}" />&nbsp;</label>
					</dd>
				</dl>
				<dl class="data-unit-dl">
					<dt>现任职务开始时间：</dt>
					<dd>
						<label title='<bean:write name="verPbPersonInfoForm" property="startDateStr" />'> <bean:write name="verPbPersonInfoForm" property="startDateStr" />&nbsp;</label>
					</dd>
				</dl> --%>
			</div>
		</div>
