...
<script src="hrinfo/bizdocument/js/BizDocumentUtils.js" type="text/javascript"></script>
<!--滚动条优化插件-->
...
<div id="transaction_modal_box" class="transaction_modal_box">
    ...
    <div class="modal-dialog-container">
        ...
        <div class="md-main-content m-file-upload">
            ...
            <span class="upload-tips">业务类型：<span>
            <select onchange="change(this.options[this.options.selectedIndex].value)" style="width:15%">
            ...
            <span class="upload-tips" style="padding-left: 15%">请上传业务材料<span>（不支持exe文件类型，不得超过5MB）</span></span> 
               ...
               <dt class="checkA_horizontal_left"><b class="Required">* </b>业务类型：</dt>
                <dd>
                    <label>
                        <select onchange="change(this.options[this.options.selectedIndex].value)">
		                	<c:if test="${!empty list}">
		                        <option value="">--请选择--</option>
								<c:forEach var="dto" items="${list}">
								<option value="${dto.refCode}">${dto.refDesc}</option>
                ...
                <button class="btn-browse-icon btn-search btn-default">浏览<input type="file" id="fileuploadDomId" class="file-choice" multiple></button>
...
(function($, window, document){
			...
			if (errorMessage.length > 0) {
				...
				BizDocumentUtils.md5({
					...
					complete : function(md5, fileName, file, options) {
						...
						var uuid = BizDocumentUtils.uuid();
						//构建url
						var url = 'createBizDocument.do?method=createBizDocument';
						url = url + '&uuid='+uuid;
						url = url + '&fileMD5='+md5;
						...
						operation.find('[button-click=upload]')
							.click(function() {
								...
								if(''==refCode){
									MessageBox.alert('提示','请先选择业务类型');
									return;
								}
								data.url = url+'&refCode='+refCode;
								data.submit();
	...
	$.get(
		'listUploadFile.do?method=listUploadFile&viewRefCodes=${param.viewRefCodes}&refCode=${param.refCode}',
		function(data){
