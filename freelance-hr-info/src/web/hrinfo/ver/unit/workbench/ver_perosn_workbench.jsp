...
$(document).ready(function() {
	...
	worktop = new Worktop([
		{
			xtype:'Xtable',
			xname:'grid',
			url: 'listVerPerson.do?method=listVerPerson&unitOid='+unitOid+'&orgOid='+orgOid+'&organizationOid='+organizationOid+'&functionCode=personWorkBench',
	...
	worktop.grid.on('draw', function(){
		...
		tabInfoTips.find("button[opt=update]").click(function(){
			...
			if (personOid&&unitOid) {
				...
				HistoryRegister.set("goBackUrl","goVerPbPersonWorkbench.do?method=goVerPbPersonWorkbench&unitOid="+unitOid+"&orgOid="+orgOid+"&organizationOid="+organizationOid+"&orgType="+orgType,values);
				...
				params.functionCode='verPersonUpdate';
				params.orgOid=orgOid;
				location.href = "goVerPersonUpdate.do?method=goToViewVerPersonPage&personOid="+personOid+"&orgOid="+orgOid+"&condition="+JSON.stringify(params);
			}
		});
		tabInfoTips.find("button[opt=view]").click(function(){
			...
			if (personOid&&unitOid) {
				...
				params.pageSize=worktop.grid.page.limit;
				params.total=worktop.grid.page.total;
				params.functionCode='verPersonView';
				params.orgOid=orgOid;
				location.href = "goVerPersonView.do?method=goToViewVerPersonPage&personOid="+personOid+"&condition="+JSON.stringify(params);
			}
		});
					...
					if(data.list.length==0){
						...
						for(var i=0;i<list.length;i++){
							...
							var photoPath = list[i].photoPath;
						   	li+='<img src="<%=request.getContextPath() %>/hrinfo/ver/unit/comm/photo_view.jsp?photoPath='+photoPath+'" alt="'+photoName+'"/>';
						}
						li+="</ul>";
						$(li).appendTo($('#imgList'));
						$('#dowebok').viewer();
	...
	worktop.grid.wrap.find('.ct').height(
      $(window).height()
          ...
          - $("#nav_tab").outerHeight(true)
          - $("#handel-button").outerHeight(true) - 115);
	$(window).resize(
      function() {
      	worktop.grid.wrap.find('.ct').height(
            $(window).height()
                ...
                - $(".search-content").outerHeight(true)
                - $("#nav_tab").outerHeight(true)
                - $("#handel-button").outerHeight(true) - 50);
      });
	...
	var settingCondition = new SettingCondition({sortFields:sortFields,functionCode: 'personWorkBench'});
	...
	$('[button-click=ver-delete]').click(function(){
	 ...
	 $('[button-click=ver-exportPbPersonInfo]').click(function(){
			 var rows = worktop.grid.selectModel.getSelectRows();
			 ...
			 if(rows.length==0) {
				 MessageBox.openWindow('exportPbPersonInfo.do?method=exportPbPersonInfo&checkFlag=N&orgOid='+orgOid+'&organizationOid='+organizationOid);
			 }else{
				 var persons = [];
				 for (var i=0; i<rows.length; i++) {
					 var person ={};
					 person.personOid=rows[i].data.personOid;
					 person.personCode=rows[i].data.personCode;
					 persons.push(person);
			   	 }
				MessageBox.openWindow('exportPbPersonInfo.do?method=exportPbPersonInfo&checkFlag=Y&persons='+JSON.stringify(persons));
			 }
	});
	 $('[button-click=uploadPersonList]').click(function() {
		Widget.openContent('goToUploadPersonListPage.do?method=goToUploadPersonListPage&isNewImport=0&unitOid='+unitOid+"&orgOid="+orgOid,function(){
			worktop.form.goQuery();
		})
	 });
...
<body style="overflow: hidden;">
	...
	<div id="right" class="personBycp">
	    ...
	    <div class="handle-btn clearfix" id="handel-button">
	        ...
	        <ul class="btn-list">
            <li class="btn-export btn-left-icon btn-default clearfix" id="export">导出
                <i class="btn-list-icon"></i>
                <ul class="btn-list-ul">
                    <li><a href="#" button-click="ver-export">导出人员名册</a></li>
                    <li><a href="#" button-click="ver-exportPbPersonInfo">导出人员信息集</a></li>
                </ul>
            </li>
            <li class="btn-print btn-left-icon btn-default clearfix">打印
	         ...
	         <button class="btn-upload btn-left-icon btn-default" button-click="uploadPersonList" >人员导入</button>
