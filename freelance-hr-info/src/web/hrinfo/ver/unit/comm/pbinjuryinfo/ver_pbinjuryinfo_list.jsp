...
<!--
 * @function    工伤信息列表页面
 * @page name   /hrinfo/ver/unit/comm/pbinjuryinfo/ver_pbinjuryinfo_list.jsp
 * @author      huangyj
 * @created     2017/02/13
 * @version     1.0
-->
...
<div class="infoshow-container padding-lrb">
  <div class="st-title-box">
    <h3 class="st-title-text">工伤信息</h3>
    <div class="st-title-icon st-title-button">
      <button class="popdown btn-add btn-left-icon btn-default btn-right" href="goToCreatePbInjuryInfoPage.do?method=goCreate&personOid=${param.personOid}&urlId=${param.Id}">新增</button>
      <button title="上传附件" class="btn-upload btn-left-icon btn-default btn-right" href="javascript:void(0);" onclick="uploadPbInjuryInfo('${param.personOid}')">上传附件</button>
    </div>
  </div>
      ...
      <tbody class="sr-table-tbody">
        <c:forEach var ="dto" items="${list}">
          <tr class="td_content">
            ...
            <td>
              <a class="st-handle-delete" href="javascript:void(0);" onclick="deletePbInjuryInfo('${dto.injuryOid}')"></a>
              <a class="st-handle-modify popdown btn" href="goToUpdatePbInjuryInfoPage.do?method=goUpdate&urlId=${param.Id}&injuryOid=${dto.injuryOid}"></a>
            </td>
          </tr>
        </c:forEach>
      </tbody>
...
function deletePbInjuryInfo(injuryOid){
	MessageBox.confirm('提示', '确认删除？',function(action)
	{
		if (action == 'yes') {
			$.ajax({
				url : 'deletePbInjuryInfo.do?method=delete',
				data :  {injuryOid:injuryOid},
				dataType : 'json',
				error : function(x,t) {
					alert(t);
					alert("error occured!!!");
...
function uploadPbInjuryInfo(personOid){
