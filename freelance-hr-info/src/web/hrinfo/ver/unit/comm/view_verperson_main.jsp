		...
		success : function(data) {
			...
			$.each(item,function(index) {
				...
				if(item[index].personOid==personOid){
					li+="<li title='"+item[index].name+"' class='name_checked'><a href='javascript:void(0);' class='nl-name-a nl-name-aselect' onclick=findPersonType(this,'"+personType+"','"+personId+"')>"+item[index].name+"</a></li>";
...
function findPageUrl(id,personId,infoUrl,flag){
	...
	$("#" + id).load(infoUrl,{"personOid":personId,"Id":id},function(){
		if(flag!==false){
			/* var pos = $("#" + id).offset(); */
			var pos = $("#" + id).position();
			$(".main-container").animate({ scrollTop: pos.top}, 1000);
	    }
 });
}
</script>
