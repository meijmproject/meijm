...
<!--
 * @function    自然减员信息列表页面
 * @page name   /hrinfo/ver/unit/comm/pbdeathinfo/ver_pbdeathinfo_list.jsp
 * @author      huangyj
 * @created     2017/02/13
 * @version     1.0
-->
...
<div class="infoshow-container padding-lrb">
  <div class="st-title-box">
    <h3 class="st-title-text">自然减员信息</h3>
    <div class="st-title-icon st-title-button">
      <button class="popdown btn-add btn-left-icon btn-default btn-right" href="goToCreatePbDeathInfo.do?method=goCreate&personOid=${param.personOid}&urlId=${param.Id}">新增</button>
      <button title="上传附件" class="btn-upload btn-left-icon btn-default btn-right" href="javascript:void(0);" onclick="uploadPbDeathInfo('${param.personOid}')">上传附件</button>
    </div>
  </div>
      ...
      <tbody class="sr-table-tbody">
        <c:forEach var ="dto" items="${list}">
          <tr class="td_content">
            ...
            <td title="<fmt:formatNumber value="${dto.smartMoney}" pattern="#.##" minFractionDigits="2"></fmt:formatNumber>">
              <fmt:formatNumber value="${dto.smartMoney}" pattern="#.##" minFractionDigits="2"></fmt:formatNumber>
            </td>
            <td title="<fmt:formatNumber value="${dto.funeralMoney}" pattern="#.##" minFractionDigits="2"></fmt:formatNumber>">
              <fmt:formatNumber value="${dto.funeralMoney}" pattern="#.##" minFractionDigits="2"></fmt:formatNumber>
            </td>
            <td>
              <a class="st-handle-delete" href="javascript:void(0);" onclick="deletePbDeathInfo('${dto.personOid}')"></a>
              <a class="st-handle-modify popdown btn" href="goToUpdatePbDeathInfo.do?method=goUpdate&urlId=${param.Id}&personOid=${dto.personOid}"></a>
            </td>
          </tr>
        </c:forEach>
      </tbody>
...
function deletePbDeathInfo(personOid){
	MessageBox.confirm('提示', '确认删除？',function(action)
	{
		if (action == 'yes') {
			$.ajax({
				url : 'deletePbDeathInfo.do?method=delete',
				data :  {personOid:personOid},
				dataType : 'json',
				error : function(x,t) {
					alert(t);
					alert("error occured!!!");
