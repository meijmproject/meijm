...
(function($, window, document){
			...
			if (errorMessage.length > 0) {
				...
				UploadPhotoUtils.md5({
					...
					complete : function(md5, fileName, file, options) {
						...
						//构建url
						var url = 'uploadPhoto.do?method=uploadPhoto&personOid=${personOid}&refType=${refType}';
						url = url + '&uuid='+uuid;
						url = url + '&fileMD5='+md5;
	...
	$.get(
		'listUploadPhoto.do?method=listUploadPhoto&personOid=${param.personOid}&refType=${param.refType}',
		function(data){
	...
	function deleteFile(photoOid, context) {
		MessageBox.yes('提示','请确认是否删除?', function(){
			$.get('deleteUploadPhoto.do?method=deleteUploadPhoto&photoOid='+photoOid, function(data) {
	...
	function downloadFile(photoOid) {
		MessageBox.openWindow('downloadUploadPhoto.do?method=downloadUploadPhoto&photoOid='+photoOid);
	};
