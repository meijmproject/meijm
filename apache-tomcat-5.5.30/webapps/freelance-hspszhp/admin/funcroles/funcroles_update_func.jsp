...
function loadTree(){
	...
	$.ajax({  
	    ...
	    success:function(data){  
	        ...
	        //eachHideNode(treeNodes); 
	        moveTreeNode(zTreeObjLeft, zTreeObjRight);
	    }  
	});  
}
function eachHideNode(nodes){
	var treeObjLeft = $.fn.zTree.getZTreeObj("treeDemoLeft");
	var treeObjRight = $.fn.zTree.getZTreeObj("treeDemoRight");
	for(var i=0;i<nodes.length;i++){
		var node= nodes[i];
	    if(node.leaf==true){
	    	if(node.isSelect==true){
	    		treeObjLeft.hideNode(treeObjLeft.getNodeByParam("funcId",node.funcId));
	    		treeObjRight.showNode(treeObjRight.getNodeByParam("funcId",node.funcId));
		    }else{
		    	treeObjLeft.showNode(treeObjLeft.getNodeByParam("funcId",node.funcId));
		    	treeObjRight.hideNode(treeObjRight.getNodeByParam("funcId",node.funcId));
			}
	    }else{
	    	var nodeschildren = node.children;
	    	eachHideNode(nodeschildren);
		}
	}
}
...
$(".addElement").click(function(){
	moveTreeNode(zTreeObjLeft, zTreeObjRight);
	if($("#func").hasClass("role_save_style")){
		$("#func span").remove();
		$("#func").removeClass().addClass("role_hit");
		$("#func").text("功能授权");
	}
});
...
$(".popover_footer_save_bt").click(function(){
	...
	var funcId="";
	for(var i=0;i<nodes.length;i++){
		funcId=nodes[i].funcId;
		funcIds+=funcId+",";
	}
	funcIds=funcIds.substr(0,funcIds.lastIndexOf(","));
	if(funcIds!=null || funcIds!=""){
		$.ajax({  
		    type:'POST',
		    dataType:'json',
		    data : {roleId:roleId,funcIds:funcIds},
		    url:"updateRoleFunction.do?method=updateRoleFunction",
		    error:function(r,t){  
		        alert(t);  
		    },  
		    success:function(data){
			    $("#func").text("");
			    $("#func").removeClass().addClass("role_save_style");
			    $("#func").append("<span class='role_hit_span'>功能授权</span>");
			    $("#func").append("<span class='Role_save_span2'></span>");
		    }  
		});  
	}
});
