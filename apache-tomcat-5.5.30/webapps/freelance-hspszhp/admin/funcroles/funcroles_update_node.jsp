...
function loadTree(){
	...
	$.ajax({  
	    ...
	    data : {roleId:roleId,menuType:2},
	    url:"findNodeTree.do?method=findItemTree",
	    error:function(){  
	        //请求失败处理函数  
	        alert('亲，请求失败！');  
	    }, 
...
$(".addElement").click(function(){
	moveTreeNode(zTreeObjLeft, zTreeObjRight);
	if($("#node").hasClass("role_save_style")){
		$("#node span").remove();
		$("#node").removeClass().addClass("role_hit");
		$("#node").text("环节授权");
	}
});
...
$(".popover_footer_save_bt").click(function(){
	...
	if(menuCodes!=null || menuCodes!=""){
		$.ajax({  
		    type:'POST',
		    dataType:'json',
		    data : {roleId:roleId,menuCodes:menuCodes,flag:'A'},
		    ...
		    success:function(data){
		    	 $("#node").text("");
				 $("#node").removeClass().addClass("role_save_style");
				 $("#node").append("<span class='role_hit_span'>环节授权</span>");
				 $("#node").append("<span class='Role_save_span2'></span>");
		    }  
		});  
	}
});
