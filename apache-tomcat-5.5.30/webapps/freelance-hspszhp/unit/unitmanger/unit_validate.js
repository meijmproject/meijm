...
*/	        email:{
        ...
        errorLabelContainer: $("#formUnitUpdate div.md-error"),
        wrapper: "span",
        submitHandler: function (form)
        {
        	    var unitOid = $('#unitOid').val();
                $.ajax({
                    url : $('#formUnitUpdate').attr("action"),
                    data :  $("#formUnitUpdate").serializeArray(),
                    dataType : 'json',
                    async : false,
                    success : function(data) {                   
                        try {
                            if (data.success) {
                                $.fn.close_popdown();
                                expandNode();
                                $('#unitInfo').load("getUbUnit.do?method=getUnitInformation",{unitOid:unitOid});
                            }
                            else
                            {
                            	$("#formUnitUpdate div.md-error").css('display','block');
                                if($("#formUnitUpdate div.md-error .back-error").length>0) {
                                	$("#formUnitUpdate div.md-error .back-error").html(data.message);
                                }else {
                                	$("#formUnitUpdate div.md-error").append("<span class='back-error'>"+data.message+"</span>");
                                }
                            }
                            return;
                        } catch (e) {
