	...
	$.ajax({
		...
		success : function(data) {
				...
				$.each(data,function(index,item){
					...
					firstLi+="<span class='ln-plus-icon'></span><a class='ln-firstli-a' href='javascript:void(0);' title='"+item.orgName+"' onclick=workUnitLocation('"+item.organizationOid+"','"+item.orgType+"')>"+item.orgName+"</a>";
				    	...
				    	$.each(item.children,function(index1,item1){
							...
							if(!item1.leaf)
							{
								 ...
								 if(index1==item.children.length-1){
									 SecondLi+=" <span class='ln-plus-icon'></span><a class='ln-secondli-a' href='javascript:void(0);' title='"+item1.orgName+"' onclick=workLocation('"+item1.orgOid+"','"+item1.orgType+"') >"+item1.orgName+"</a> ";
								 }else{
									 SecondLi+=" <span class='ln-plus-icon'></span><a class='ln-secondli-a' href='javascript:void(0);' title='"+item1.orgName+"' onclick=workLocation('"+item1.orgOid+"','"+item1.orgType+"') >"+item1.orgName+"</a>";
								 }
							}else{
								...
								$.each(item1.children,function(index2,item2){
									...
									if(!item2.leaf)
									{
										 ...
										 if(index2==item1.children.length-1){
											 TrirdLi+=" <span class='ln-plus-icon'></span><a class='ln-thirdli-a' href='javascript:void(0);' title='"+item2.orgName+"' onclick=workLocation('"+item2.orgOid+"','"+item2.orgType+"')>"+item2.orgName+"</a>";
										 }else{
											 TrirdLi+=" <span class='ln-plus-icon'></span><a class='ln-thirdli-a' href='javascript:void(0);' title='"+item2.orgName+"' onclick=workLocation('"+item2.orgOid+"','"+item2.orgType+"')>"+item2.orgName+"</a>";
										 }
									}else{
										TrirdLi+="<li class='ln-thirdli'><span class='ln-line'></span>";
										 if(index2==item1.children.length-1){
											 TrirdLi+=" <a class='ln-thirdli-a' href='javascript:void(0);' title='"+item2.orgName+"' onclick=workLocation('"+item2.orgOid+"','"+item2.orgType+"')>"+item2.orgName+"</a>";
										 }else{
											 TrirdLi+=" <a class='ln-thirdli-a' href='javascript:void(0);' title='"+item2.orgName+"' onclick=workLocation('"+item2.orgOid+"','"+item2.orgType+"')>"+item2.orgName+"</a>";
										 }
									}
										...
										$.each(item2.children,function(index3,item3){
											...
											if(!item3.leaf)
											{
												 ...
												 if(index3==item2.children.length-1){
													 ForthLi+=" <span class='ln-plus-icon'><a class='ln-fourthli-a' href='javascript:void(0);' title='"+item3.orgName+"' onclick=workLocation('"+item3.orgOid+"','"+item3.orgType+"') >"+item3.orgName+"</a>";
												 }else{
													 ForthLi+=" <span class='ln-plus-icon'><a class='ln-fourthli-a' href='javascript:void(0);' title='"+item3.orgName+"' onclick=workLocation('"+item3.orgOid+"','"+item3.orgType+"') >"+item3.orgName+"</a>";
												 }
											}else{
												ForthLi+="<li class='ln-fourthli'><span class='ln-line'></span>";
												 if(index3==item2.children.length-1){
													 ForthLi+=" <a class='ln-fourthli-a' href='javascript:void(0);' title='"+item3.orgName+"' onclick=workLocation('"+item3.orgOid+"','"+item3.orgType+"') >"+item3.orgName+"</a>";
												 }else{
													 ForthLi+=" <a class='ln-fourthli-a' href='javascript:void(0);' title='"+item3.orgName+"' onclick=workLocation('"+item3.orgOid+"','"+item3.orgType+"') >"+item3.orgName+"</a>";
												 }
											}
		    ...
		    //默认显示右侧工作台  
			$(".right-content").load("goToPersonWorkBench.do?method=goToPersonWorkBench&unitOid="+unitOid+"&orgOid="+orgOid+"&orgType="+orgType);
		}
	});
}
...
function workLocation(orgOid,orgType)
{
	$(".right-content").load("goToPersonWorkBench.do?method=goToPersonWorkBench&orgOid="+orgOid+"&orgType="+orgType);
}
function workUnitLocation(organizationOid,orgType)
{
	$(".right-content").load("goToPersonWorkBench.do?method=goToPersonWorkBench&organizationOid="+organizationOid+"&orgType="+orgType);
}
</script>
</body>
</html>