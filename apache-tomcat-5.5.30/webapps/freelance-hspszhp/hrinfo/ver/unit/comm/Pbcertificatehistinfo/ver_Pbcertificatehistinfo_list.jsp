...
<!--
 * @function    执业注册历史信息列表页面
 * @page name   /hrinfo/ver/unit/comm/Pbcertificatehistinfo/ver_Pbcertificatehistinfo_list.jsp
 * @author      huangyj
 * @created     2017/02/13
 * @version     1.0
-->
...
<div class="infoshow-container padding-lrb">
  <div class="st-title-box">
    <h3 class="st-title-text">执业注册历史信息</h3>
    <div class="st-title-icon st-title-button">
      <button class="popdown btn-add btn-left-icon btn-default btn-right" href="goToCreatePbCertificateHistInfoPage.do?method=goCreate&personOid=${param.personOid}&urlId=${param.Id}">新增</button>
      <button title="上传附件" class="btn-upload btn-left-icon btn-default btn-right" href="javascript:void(0);" onclick="uploadPbCertificateHistInfo('${param.personOid}')">上传附件</button>
    </div>
  </div>
      ...
      <tbody class="sr-table-tbody">
        <c:forEach var ="dto" items="${list}">
          <tr class="td_content">
            ...
            <td>
              <a class="st-handle-delete" href="javascript:void(0);" onclick="deletePbCertificateHistInfo('${dto.certificateHistOid}')"></a>
              <a class="st-handle-modify popdown btn" href="goToUpdatePbCertificateHistInfoPage.do?method=goUpdate&urlId=${param.Id}&certificateHistOid=${dto.certificateHistOid}"></a>
            </td>
          </tr>
        </c:forEach>
      </tbody>
...
function deletePbCertificateHistInfo(certificateHistOid){
	MessageBox.confirm('提示', '确认删除？',function(action)
	{
		if (action == 'yes') {
			$.ajax({
				url : 'deletePbCertificateHistInfo.do?method=delete',
				data :  {certificateHistOid:certificateHistOid},
				dataType : 'json',
				error : function(x,t) {
					alert(t);
					alert("error occured!!!");
...
function uploadPbCertificateHistInfo(personOid){
