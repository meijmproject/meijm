...
<div class="infoshow-container padding-lrb">
        <div class="st-title-box"><h3 class="st-title-text">职业资格信息</h3>
	       <div class="st-title-icon st-title-button">
	           <button class="popdown btn-add btn-left-icon btn-default btn-right" href="goToCreatePbQualificationsInfoPage.do?method=goCreate&personOid=${param.personOid}&urlId=${param.Id}">新增</button>
	           <button title="上传附件" class="btn-upload btn-left-icon btn-default btn-right" href="javascript:void(0);" onclick="uploadQualificationsInfo('${param.personOid}')">上传附件</button>
	       </div>
        </div>
             ...
             <tbody class="sr-table-tbody">
              <c:forEach var ="dto" items="${list}" varStatus="status">
                <tr>
                    ...
                    <td>
                    <a class="st-handle-delete" href="javascript:void(0);" onclick="deleteQualificationsInfo('${dto.qualificationsOid}')"></a>
                    <a class="st-handle-modify popdown btn" href="goToUpdatePbQualificationsInfoPage.do?method=goUpdate&urlId=${param.Id}&qualificationsOid=${dto.qualificationsOid}"></a>
                    </td>
                </tr>
                </c:forEach>
             </tbody>
...
<%-- <div class="sys_box sys_box_up">
        <div class="sys_base"><span>执业(执业)资格信息</span>
            <a style="float:right;margin-left: 20px;" href="goToCreatePbQualificationsInfoPage.do?method=goCreate&personOid=${param.personOid}&urlId=${param.Id}" class="popdown btn"><img src="img/DetailPages/icon_add_green.png"/></a>
        </div>
        <div class="sys_list">
            <table class="table_edit_bg" width="95%">
                ...
                <tr class="td_content">
                	...
                	<td title="${dto.validityDateStr}">${dto.validityDateStr }</td>
                    <td>
                    	<a href="javascript:void(0);" onclick="deleteQualificationsInfo('${dto.qualificationsOid}')"><img src="img/DetailPages/icon_delete.png"/></a>
                        <a href="goToUpdatePbQualificationsInfoPage.do?method=goUpdate&urlId=${param.Id}&qualificationsOid=${dto.qualificationsOid}" class="popdown btn"><img src="img/DetailPages/icon_revise.png"/></a>
                    </td>
                </tr>
                </c:forEach>
            </table>
...
function updateEducationInfo(qualificationsOid){
	...
	$.ajax({
		url : 'updatePbEducationInfo.do?method=update&qualificationsOid='+qualificationsOid+'&personOid=${personOid}',
...
function goUpdateEducationInfo(qualificationsOid){
	$.ajax({
		url : 'goUpdatePbEducationInfo.do?method=goUpdate',
		data :  {qualificationsOid:qualificationsOid},
		dataType : 'json',
		error : function(x,t) {
			alert(t)
			alert("error occured!!!");
		...
		success : function(data) {
			if (data) {
				...
				$("#u_qualificationsOid").val(data.qualificationsOid);
...
function deleteQualificationsInfo(qualificationsOid){
	MessageBox.confirm('提示', '确认删除？',function(action)
	{
		if (action == 'yes') {
			$.ajax({
				url : 'deletePbQualificationsInfo.do?method=delete',
				data :  {qualificationsOid:qualificationsOid},
				dataType : 'json',
				error : function(x,t) {
					alert(t);
...
function uploadQualificationsInfo(personOid){
